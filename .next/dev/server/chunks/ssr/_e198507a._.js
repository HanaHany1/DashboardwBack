...
const apiClient = {
    ...
    async getRooms () {
        try {
            ...
            if (!res.ok) {
                console.log("[v0] /api/rooms endpoint failed with status:", res.status);
                return [];
            }
            const data = await res.json();
            return Array.isArray(data) ? data : data.rooms || data.data || [];
        } catch (error) {
            console.log("[v0] Rooms fetch error:", error);
            return [];
        }
    },
    ...
    async getRoomsByBranch (branchId) {
        const res = await fetch(`${API_BASE_URL}/api/rooms/branch/${branchId}`);
...
//# sourceMappingURL=_e198507a._.js.map